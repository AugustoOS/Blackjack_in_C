...
// Retorna 1 se ainda existe alguma carta disponível no baralho.
int temCarta(){
...
void baralho(){
	...
	// Recompõe o baralho global: sem isso as cartas marcadas como usadas
...
void compra(){
	...
	// O sorteio sempre respeita as cartas já usadas e para quando o baralho acaba.
	...
	do{
		...
		if (resposta == 's' || resposta == 'S'){
			...
			// A regra permite comprar no máximo 5 cartas.
			if (j == MAX_COMPRAS) printf("\n--> Você atingiu o limite de %d cartas compradas.", MAX_COMPRAS);
...
int resultado(){
	...
	if (valorP1 == valorPC || (valorP1 > 21 && valorPC > 21)){
		...
		else if (valorP1 > 21 && valorPC > 21) printf("\n\tVocês empataram! Ambos ultrapassaram o valor 21. Seu valor foi %d e o do computador %d.", valorP1, valorPC);
